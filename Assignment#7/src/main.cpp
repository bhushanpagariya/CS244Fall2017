...
void setup()
{
  ...
  WiFiClient client;
  if (!client.connect("ec2-35-167-126-243.us-west-2.compute.amazonaws.com", 80)) {
    Serial.println("Failure");
  }

  client.println("POST /main.php HTTP/1.1");
  client.println("Host: ec2-35-167-126-243.us-west-2.compute.amazonaws.com");
  client.println("Content-Type: multipart/form-data");
  client.print("Content-Length: ");
  client.println(1100000);
  client.println();
  ...
  while(index<400){
    ...
    while(currentSample < maxSample) {
      ...
      sprintf(tempStr+start, "%d,%d,%d,",(millis()-currentTime), ir_val, red_val);
